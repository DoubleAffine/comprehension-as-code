...
<task type="auto">
  ...
  <action>
    ...
    - test_observation_refs_retained_after_prune: After mark_content_pruned(), the comprehension.observations list still contains the observation ID (validates key memory-efficiency principle: references persist even when content is deleted)
  </action>
  <verify>pytest tests/test_observation_index.py -v && python -c "
# Explicit verification: observation references remain after content pruning
...
comp = Comprehension(
    ...
    domain='test',
    prior=BeliefPrior(statement='Init', confidence=ConfidenceLevel.UNKNOWN, source='test'),
    observations=['obs-A', 'obs-B'],
    posterior=BeliefPosterior(statement='Updated', confidence=ConfidenceLevel.MEDIUM, update_reasoning='Test', observations_used=['obs-A']),
...
assert not obs_index.is_content_available('obs-A'), 'Content should be marked pruned'
print('STORE-04b verified: observation references retained after content pruned')
os.unlink(db_path)
"</verify>
  <done>ObservationIndex tracks references and pruning status; integrates with ObservationLifecycle; observation references in comprehension.observations persist after content pruning</done>
</task>
